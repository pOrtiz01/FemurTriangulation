...
def distance(vertex1, vertex2)  :  # distance between 2 points
    x = vertex2.coords[0] - vertex1.coords[0]
    y = vertex2.coords[1] - vertex1.coords[1]
    z = vertex2.coords[2] - vertex1.coords[2]
    dist = math.sqrt(x * x + y * y + z * z  )  # absolute value of differences
    return dist


def area(vertex1, vertex2, vertex3):  # calculate area
    vector1 = subtract(vertex1.coords, vertex2.coords  )  # get two vectors
    vector2 = subtract(vertex1.coords, vertex3.coords)
    result = crossProduct(vector1, vector2  )  # cross product the vectors
    triArea = pow((pow(result[0], 2) + pow(result[1], 2) + pow(result[2], 2)), 0.5  )  # calculate area by getting absolute value
    triArea = 0.5 * triAre  a  # divide by two to get triangle area
...
def buildTriangles(slice0, slice1):
    ...
    for i in range(len(slice1.verts))  :  # loop through all pairs of vertices, brute force, to find min distance
        for j in range(len(slice0.verts)):
            if distance(slice0.verts[j], slice1.verts[i]) >= minDistance:
                continue# if distance is bigger skip iteration
            elif distance(slice0.verts[j], slice1.verts[i]) < minDistance:
                minDistance = distance(slice0.verts[j], slice1.verts[i])
                index1 =  i  # save indices of closest vertices
                index2 = j
    ...
    for i in range(index2, index2 + len(slice0.verts)):  # make permutation starting at closest pair of vertices
        perm0.append(slice0.verts[i % len(slice0.verts)])

    for i in range(index1, index1 + len(slice1.verts)):
        perm1.append(slice1.verts[i % len(slice1.verts)])

    perm0.append(slice0.verts[index2])  # add first vertex to end of permutation
    perm1.append(slice1.verts[index1])
    ...
    minArea = [[0 for i in range(len(perm0))] for j in
               range(len(perm1))]  # setup minarea and mindir, fill minarea with 0s
    minDir = [[None for i in range(len(perm0))] for j in range(len(perm1))]  # fill mindir with None

    # Fill in the minArea array

    minArea[0][0] = 0.0  # Starting edge has zero area

    # Fill in row 0 of minArea and minDir, since it's a special case as there's no row -1
    #
    # [2 marks]
    prev = 0.0
    for i in range(1, len(perm0)):
        minArea[0][i] = area(perm1[0], perm0[i - 1],
                             perm0[i]) + prev  # special case area plus previous area horizontally
    ...
    for i in range(1, len(perm1)):
        minArea[i][0] = area(perm0[0], perm1[i - 1], perm1[i]) + prev  # special case area plus previous area vertically
        prev = minArea[i][0]
    ...
    for i in range(1, len(perm1)):
        for j in range(1, len(perm0)):
            temp1 = area(perm0[j], perm1[i], perm1[i - 1])#calculate area moving up
            temp2 = minArea[i - 1][j] + temp1#area above
            temp3 = area(perm1[i], perm0[j], perm0[j - 1]) #if moving left
            temp4 = minArea[i][j - 1] + temp3#area to the left
            #if coming from the cell above current cell
            if temp1 + temp2 < temp3 + temp4:
                minArea[i][j] = temp1 + temp2
                #coming from the cell above current itteration
                minDir[i][j] = Dir.PREV_ROW
            else:
                minArea[i][j] = temp3 + temp4
                #coming from the cell beside current itteration
    ...
    #
    # [3 marks]
    i = len(perm1) - 1  # start at end
    j = len(perm0) - 1
    triangles = []

    while (i != 0 or j != 0):  # while we dont reach the first square of minArea
        tempVerts = []  # temp array
        if (minDir[i][j] == Dir.PREV_ROW):  # if mindir says move up
            tempVerts.append(perm1[i])  # append to temp array moving up
            tempVerts.append(perm0[j])
            tempVerts.append(perm1[i - 1])
            temp = Triangle(tempVerts)  # create triangle with temp array
            i = i - 1  # adjust index to move up
        else:
            tempVerts.append(perm1[i])  # append to temp array moving left
            tempVerts.append(perm0[j])
            tempVerts.append(perm0[j - 1])
            temp = Triangle(tempVerts)  # create triangle with temp array
            j = j - 1  # adjust index to move left
        triangles.append(temp)  # append new triangle to triangle list

    if (minDir[i][
        j] == Dir.PREV_ROW):  # last case is not hit by above while loop so this handles that case in the same way
        # tempVerts.append(perm1[i],perm0[j],perm1[i-1])
        tempVerts.append(perm1[i])
        tempVerts.append(perm0[j])
        tempVerts.append(perm1[i - 1])
        temp = Triangle(tempVerts)
    else:
        tempVerts.append(perm1[i])
        tempVerts.append(perm0[j])
        tempVerts.append(perm0[j - 1])
